...
public class SecondRoomState : TutorialIState
{
    ...
    public void UpdateState(TutorialController sc)
    {
        ...
        if (sc.gc.currentHitObject != null)
        {
            ...
            {
                sc.gc.helperText.text = "Click to sign the Contract";
                sc.gc.helperText.enabled = true;
                if (Input.GetKeyDown(KeyCode.Mouse0) && sc.gc.currentHitObject.name == "Contract" && Physics.Raycast(ray, out hit, 5f, sc.gc.layMask))
                {
                    sc.gc.tutText.text = "";
