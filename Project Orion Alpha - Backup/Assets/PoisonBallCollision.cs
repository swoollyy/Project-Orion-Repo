    ...
    void Update()
    {
        ...
        if(timer >= 15f)
        {
            Destroy(this.gameObject);
                ...
                if (Vector3.Distance(transform.position, player.transform.position) > 5f && !inRange)
                {
            doOnce = false;
                    transform.position = Vector3.MoveTowards(transform.position, player.transform.position, Time.deltaTime * 6f);
                ...
                {
                    inRange = true;
            transform.position += savedDirection * 6f * Time.deltaTime;
                }
