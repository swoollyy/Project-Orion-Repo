...
public class water : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!isMoving)
        {
            ...
            {
                if (isAtTop || isAtMiddle)
                    randomMax = Random.Range(10f, 20f);
                else
                    randomMax = Random.Range(25f, 65f);
            }
        ...
        {
            forcingBottom = true;
            StopCoroutine(RiseToMiddle());
            StopCoroutine(RiseFromFloorToTop());
            StopCoroutine(RiseToTop());
            StopCoroutine(FallToFloor());
            StopCoroutine(FallToMid());
            StopCoroutine(FallToFloorFromTop());
            timer = 0;
            randomMoveTimer = 0;
            StartCoroutine(FallToFloorWherever());
            forceBottom = false;
        }
    ...
    private IEnumerator RiseToMiddle()
    {
        ...
        while (timer < riseTime && !forcingBottom);
    ...
    private IEnumerator RiseFromFloorToTop()
    {
        ...
        while (timer < (riseTime *2) && !forcingBottom);
    ...
    private IEnumerator  FallToFloorWherever()
    {
        Vector3 minScale = new Vector3(waterObj.transform.localScale.x, currentSize, waterObj.transform.localScale.z);
        Vector3 maxScale = new Vector3(waterObj.transform.localScale.x, minSizeFloor, waterObj.transform.localScale.z);
